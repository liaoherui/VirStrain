...
with open(snp_kmr_file, 'r') as f1:
	while True:
...
with open(matrix_file, 'r') as f3:
	...
	freq_arr = []
	with open('Tem_Vs_' + uid + '.fa', 'r') as fnew:
		while True:
	...
	if np.sum(freq_arr) == 0:
		print('No kmers remained after depth filtering! No virus strain can be detected!')
		sys.exit(0)
...
if len(dmr) == 0:
	print('No strain profiles available after matrix scoring!')
	sys.exit(0)
...
if len(res) == 0:
	print('No strain candidates available!')
	sys.exit(0)
...
while True:
	...
	keep = (pre_freq_arr != 0)
	pre_freq_arr = pre_freq_arr[keep]
	if len(pre_freq_arr) > 0 and np.sum(pre_freq_arr) != 0:
		pre_wf_arr = pre_freq_arr / np.sum(pre_freq_arr)
...
if np.sum(left_freq_arr) == 0:
	left_weighted_freq_arr = left_freq_arr
else:
...
if len(left_freq_arr) != 0:
	...
	for l in range(max_iter_times):
		...
		for r in res:
			if r[1] == res[0][1]:
				...
				if len(nt) == 0:
					...
					if len(nt[keep]) != 0:
						nt = nt[keep]
					ds_avgd[r[0]] = np.mean(nt) if len(nt) != 0 else 0
				## Done
		...
		if len(left_freq_arr) != 0 and np.sum(left_freq_arr) != 0:
			left_weighted_freq_arr = left_freq_arr / np.sum(left_freq_arr)
...
if len(candidate_cls) != 0:
	with open(sub_kmr_file, 'r') as fsk:
		ksub = {}
		cls_sub = {}
		# subcount={}
		while True:
	...
	with open('Tem_Vs2Sub_' + uid + '.fa', 'w+') as ok:
		co = 1
		for kmr in ksub:
			ok.write('>' + str(co) + '\n')
			ok.write(kmr + '\n')
	...
	with open('Tem_Vs2_' + uid + '.fa', 'r') as ft2:
		while True:
