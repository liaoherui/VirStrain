...
def cls_with_perl_hier(sstrain2seq, strain_set, src_path):
	...
	with open('Vbuild_tem_cls.aln', 'w+') as fs:
		for s in sstrain2seq:
			fs.write(s + '\n' + sstrain2seq[s] + '\n')
			cnum = len(sstrain2seq[s])

	os.system(
		'perl ' + src_path + '/aln2cluster-overlap-kmer-withd.pl '
		+ 'Vbuild_tem_cls.aln ' + str(len(strain_set)) + ' 0 0 '
		+ str(cnum) + ' 0 > Vbuild_tem.column'
	)
	# print('perl aln2cluster-overlap-kmer-withd.pl Vbuild_tem_cls.aln '+str(len(strain_set))+' 0 0 '+str(cnum)+' 0 > Vbuild_tem.column')
	# exit()

	with open('Vbuild_tem.column', 'r') as fs2:
		while True:
	...
	if len(selected_columns) == 0:
		...
		for c in carr:
			for s in strain_set:
				...
				if c - half_k < 0:
					block_seq = sstrain2seq[s][0:c] + base + sstrain2seq[s][c + 1:c + 1 + half_k]
				else:
					'''
					...
					exit()
					'''
					block_seq = sstrain2seq[s][c - half_k:c] + base + sstrain2seq[s][c + 1:c + 1 + half_k]
