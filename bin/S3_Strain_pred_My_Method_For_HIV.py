...
id2cls = {}
with open(input_cls_file, 'r') as fc:
	while True:
...
with open(db_dir + '/dscl.json', 'r') as fp:
	dscl = json.load(fp)
with open(db_dir + '/dscf.json', 'r') as fp:
	dscf = json.load(fp)
with open(db_dir + '/dks.json', 'r') as fp:
	dks = json.load(fp)
with open(db_dir + '/dss.json', 'r') as fp:
	dss = json.load(fp)
with open(db_dir + '/dskc.json', 'r') as fp:
	dskc = json.load(fp)
...
if os.path.exists(db_dir + '/head_file.txt'):
	with open(db_dir + '/head_file.txt', 'r') as fp:
		while True:
			line = fp.readline().strip()
			if not line:
				break
...
with open(out_file, 'w+') as o:
	...
	if len(res) == 0:
		...
		if id2name[res[0][0]] not in dc:
			o.write('\t\t' + id2name[res[0][0]] + '\t' + id2cls[res[0][0]] + '\t' + str(res[0][1]) + '\t' + str(len(dsvkc.get(res[0][0], {}))) + '/' + str(len(dscl[res[0][0]])) + '\t' + str(depth_mp) + '\tNA\n')
		else:
			o.write('\t\t' + id2name[res[0][0]] + '\t' + id2cls[res[0][0]] + '\t' + str(res[0][1]) + '\t' + str(len(dsvkc.get(res[0][0], {}))) + '/' + str(len(dscl[res[0][0]])) + '\t' + str(depth_mp) + '\t' + dc[id2name[res[0][0]]] + '\n')
		...
		while True:
			...
			if id2name[res[0][0]] not in dc:
				o.write('\t\t' + str(id2name[res[0][0]]) + '\t' + id2cls[res[0][0]] + '\t' + str(res[0][1]) + '\t' + str(len(dsvkc.get(res[0][0], {}))) + '/' + str(len(dscl[res[0][0]])) + '\t' + str(cdepth) + '\tNA\n')
			else:
				o.write('\t\t' + str(id2name[res[0][0]]) + '\t' + id2cls[res[0][0]] + '\t' + str(res[0][1]) + '\t' + str(len(dsvkc.get(res[0][0], {}))) + '/' + str(len(dscl[res[0][0]])) + '\t' + str(cdepth) + '\t' + dc[id2name[res[0][0]]] + '\n')
...
with open('Mps_ps_depth.csv', 'w+') as ov1, open('Ops_ps_depth.csv', 'w+') as ov2:
	if mp_sid is None:
		ov1.write('ID,Column_ID,None_Freq,None_LNum\n')
		ov2.write('ID,Column_ID,None,None\n')
	else:
		...
		if len(op_strain) == 0:
			...
			for c in carr:
				...
				for s in op_sid:
					has_dscf = c in dscf[s]
					has_dscl = c in dscl[s]

					if has_dscf and has_dscl:
						ov2.write(',' + str(dscf[s][c]) + ',' + str(dscl[s][c]))
					elif (not has_dscf) and has_dscl:
						ov2.write(',0,' + str(dscl[s][c]))
					elif has_dscf and (not has_dscl):
