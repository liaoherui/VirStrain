...
if len(pre_freq_arr) == 0 or np.sum(pre_freq_arr) == 0:
...
if len(left_freq_arr) == 0 or np.sum(left_freq_arr) == 0:
...
if len(candidate_cls) != 0:
	...
	with open('Tem_Vs2Sub.fa', 'w+') as ok:
		co = 1
		for kmr in ksub:
			ok.write('>' + str(co) + '\n')
			ok.write(kmr + '\n')
...
for c in carr:
	...
	for s in vs_sd:
		if c in dscf[s] and c in dscl[s]:
