...
sf2=pd.read_csv('Ops_ps_depth.csv')
#print(sfl.columns)
# Get 
s=[]
...
for sw in sfl[s[0]+'_Freq']:
	if sw==0: 
		...
		zy.append(sw)
	i+=1
ft=open('VirStrain_report.html','w+')
fig=go.Figure()
...
if not len(so)==0:
	...
	for s in so:
		...
		fig=go.Figure()
		fig=make_subplots(specs=[[{"secondary_y": True}]])
		fig.add_trace(go.Bar(x=sf2['ID'],y=sf2[s+'_Freq'],name='depth'),secondary_y=False,)
		fig.add_trace(go.Scatter(x=sf2['ID'],y=(-1)*sf2[s+'_LNum'],mode='lines+markers',opacity=0.5,marker=dict(size=5),name='(-1)*(strain number)'),secondary_y=True,)
		fig.add_trace(go.Scatter(x=zx,y=zy,mode='markers',opacity=0.5,marker=dict(size=5,color='red'),name='Zero depth pos'),secondary_y=False,)
		ma=sf2[s+'_LNum'].max()
		ma2=sf2[s+'_Freq'].max()
		fig.layout.update(autosize=False,width=1200,height=300,title={'text':'Other Possible Strain: '+s+' (Rank:'+str(r)+')','xanchor': 'center'})
		r+=1
		fig.update_yaxes(range=[(-3)*ma,3*ma],secondary_y=True,)
		fig.update_yaxes(range=[(-1)*ma2,ma2],secondary_y=False,)
		ft.write(fig.to_html(full_html=False, include_plotlyjs='cdn'))
